...
As part of this exercise, I am analyzing the details of baseball players and their batting averages. The sample data consists of 1157 records with below details
...
ggplot(data= nba, aes(x=as.factor(nba$handedness),
  ...
  scale_y_continuous(labels = percent)+
   scale_fill_discrete(name = "handedness by color")+
...
ggpairs(nba[, 2:6], lower=list(continuous="smooth", params=c(colour="blue")),
  diag=list(continuous="bar", params=c(colour="blue")), 
  upper=list(params=list(corSize=3)), axisLabels='show')
```

With this exercise I am going to find out the individual performance of a baseball player and how the different parameters are influencing the same.
Individual performance can be assessed by checking the batting average.

From the corelation graph:


Height vas Batting Average: -0.374
...
ggplot(data = nba, 
   ...
   geom_point(alpha = 1/2, position = position_jitter(h = 0), size = 4) +
...
coord_cartesian(ylim=c(0.066, 0.300
                          )) 
...
Hence I can reject the the null hypothesis which asserts that the means of the two samples are identical.From the R graph we have seen that average of L batsman are higher compared to R of same population. 
